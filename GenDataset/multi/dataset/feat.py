...
def list_to_str(list_to_str: list) -> str:
    ret = ""
    for s in list_to_str:
        ret += str(s)
    return ret


class FeatColumn(object):
    def __init__(self, table_name: str, col_name: str, df_col: pd.DataFrame, ref_col=None):
        self.name = col_name
...
def get_corr(columns_dict, table_name):
    table_feature = {}
    col_name_done = []
    ...
    for col_name1 in col_names:
        for col_name2 in col_names:
            if col_name1 + col_name2 in col_name_done:
                ...
                table_feature[f"sf_co_{table_name}_{col_name1}_2_{col_name2}"] = corr[col_name1][col_name2]
                col_name_done.append(col_name1 + col_name2)
    return table_feature


class Feature(object):
    def __init__(self, df_data: pd.DataFrame, df_relative: pd.DataFrame, df_total: pd.DataFrame, table_name:str):
        self.table_name = table_name
        self.total_columns_dict = {}
        self.rel_columns_dict = {}
        self.data_columns_dict = {}
        for col in df_total.columns:
            self.total_columns_dict[col] = FeatColumn(self.table_name, col, df_total[col])
            self.data_columns_dict[col] = FeatColumn(self.table_name, col, df_data[col], self.total_columns_dict[col])
            self.rel_columns_dict[col] = FeatColumn(self.table_name, col, df_relative[col], self.total_columns_dict[col])
    ...
    def feature(self):
        ...
        for col, _ in self.total_columns_dict.items():
            ...
            feat_data_dict.update(get_corr(self.data_columns_dict, self.table_name))
            feat_rel_dict.update(self.rel_columns_dict[col].gen_feat())
            feat_rel_dict.update(get_corr(self.rel_columns_dict, self.table_name))

        for k, v in feat_rel_dict.items():
            feat_dict[k] = v - feat_data_dict[k]
        L.info(f"Table {self.table_name} Generated feature size: {len(feat_dict)}")
        return feat_dict
